...
async def next_page(bot, query):
    ...
    files, n_offsett, total = await get_filter_results(search, offsett=offsett, filter=True)
    try:
...
async def auto_filter(client, msg, spoll=False):
    if not spoll:
        ...
        if 2 < len(message.text) < 50:
            ...
            files, offsett, total_resultss = await get_filter_results(search.lower(), offsett=0, filter=True)
            if not files:
