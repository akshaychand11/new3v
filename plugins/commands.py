...
#New_Verifie_User"""
    ...
    dl = await client.send_cached_media(
        ...
        caption=f_caption,
        protect_content=True if pre == 'filep' else False,
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(' 𝐉𝐨𝐢𝐧 𝙂𝙧𝙤𝙪𝙥 ', url=malik.grp_it_lk)],[InlineKeyboardButton("⚙ DEPLOY YOURS", url=malik.dp_yrs),]]),parse_mode=enums.ParseMode.HTML)
...
@Client.on_message(filters.command('shortener_api'))
async def shortener_api(client, message):
    sts = await message.reply("Checking template")
    userid = message.from_user.id if message.from_user else None
    if not userid:
        return await message.reply(f"You are anonymous admin. Use /connect {message.chat.id} in PM")
    ...
    if len(message.command) < 2:
        return await sts.edit("No Input!!")
    shortener_api = message.text.split(" ", 1)[1]
    await save_group_settings(grp_id, 'shortener_api', shortener_api)
    await sts.edit(f"Successfully changed shortener_api for {title} to\n\n{shortener_api}")
