...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb and imdb.get('poster'):
        try:
            ...
            an = await message.reply_photo(photo=poster, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(1790)
            await an.delete()
            await msg.delete()
        except Exception as e:
            logger.exception(e)
            abn = await message.reply_photo(photo=SMART_PIC, caption=cap, reply_markup=InlineKeyboardMarkup(btn))    
            await asyncio.sleep(1790)
            await abn.delete()
            await msg.delete()    
    else:
            abb = await message.reply_photo(photo=SMART_PIC, caption=cap, reply_markup=InlineKeyboardMarkup(btn))
            await asyncio.sleep(1790)
            await abb.delete()
            await msg.delete()
    if spoll:
