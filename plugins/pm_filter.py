...
async def req_grp_results(bot, msg):
    ...
    if not files: return
    await msg.reply(f'Dear {msg.from_user.mention}!, {total_results} results are already available for your query!', quote = True), reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🚶 Back to Group 🚶",url="https://t.me/+FAgX05kGByNkZjJl"),]]),parse_mode=enums.ParseMode.HTML)
    #text=(MBGH..format(message.from_user.mention, total_results', quote = True))),
